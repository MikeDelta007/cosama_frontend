...
import { TypeReglement } from 'src/app/model/TypeReglement.model';
...
export class ReglementsComponent implements OnInit {
  ...
  getFacturesByState : any;

  getFacturesByState_ : any[] = [];
  ...
  ngOnInit() {
    ...
    this.clientEnCompteService.getFacturesParEtat().subscribe((response:any) => 
      {
        this.getFacturesByState = response;
            this.getFacturesByState_ = Object.entries(this.getFacturesByState).map(([statut, factures]) => ({
                statut,
                factures
            }));
            console.log(this.getFacturesByState_);
      }
    );

    this.clientEnCompteService.getTypeReglements().subscribe((response:any) => 
        {
            this.TypeReglements = response;
            console.log(this.TypeReglements);
        }
      );
  ...
  getRaisonSocialById(id: number): string | undefined {
    const client = this.clientEnComptes.find(c => c.cltcmptId === id);
...
// }
  ...
  savePayment()
  {
    ...
    const factureUpdate_bis = {
      cltfct_id: this.facturationEdit.cltfct_id,
      montant_verse : this.facturationEdit.montant_verse,
      ref_paieFacture : this.facturationEdit.ref_paieFacture
    };

    console.log(`Facture à payer :`, factureUpdate_bis);
    
    this.clientEnCompteService.updatePayment(this.facturationEdit.cltfct_id, factureUpdate_bis, this.user).subscribe({
