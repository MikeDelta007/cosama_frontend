...
export class PaiementComponent implements OnInit {
  ...
  printTicketPassager__(oneFret: any) 
  {
    console.log(oneFret);
    this.oneCltFretEdit = { ...oneFret };
    // maintenant les lignes sont dans ligneFrets
    this.ligneFret3 = (this.oneCltFretEdit as any).ligneFrets;
    console.log(this.ligneFret3);
    this.outBillet2_ = true;
  }
  ...
  calculateTotal3_(): number {
    ...
    console.log(this.ligneFret3);
      ...
      this.ligneFret3.forEach((ligne:any) => {
        const prixUnitaire = this.getBagagePU(ligne.categorie.catId); // Récupère le prix unitaire pour le tbg_id
        const unite = this.getUniteBagage(ligne.typeBagage.tbgId);
        console.log(prixUnitaire);
        console.log(unite);
        if (prixUnitaire !== undefined) {
          if (unite === 1)
  ...
  async validerOps()
  {
      ...
      this.fret.doPayment(this.oneCltFretEdit.fretId, this.oneCltFretEdit.cltcmpt_id, this.onePayment).subscribe(
        ...
        next: response => {
          ...
          this.printTicketPassager__(response);
        },
        error: error => {
