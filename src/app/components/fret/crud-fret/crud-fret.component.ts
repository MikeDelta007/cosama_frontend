...
public oneCltFretEdit_ : FretDTO = {
    ...
    this.fret.submitFret(this.oneCltFret, this.selectedCltCmpt, this.v.voy_id, this.idBillet, this.checked).subscribe(
      ...
      next: response => {
        ...
        else
        {
          ...
          this.printTicketFret2(response);
        }
  ...
  printTicketFret2(response: any) {

  const fret = response.fret;
  const fretClient = response.fretClient;

  this.oneCltFretEdit = { ...fret };

  console.log(response);

  this.ligneFret3 = fret.ligneFrets;

  console.log(this.ligneFret3);

  this.outBillet = true;

  this.getExpediteur(fret.fretId);

  this.qrCodeUrl$ = this.billetService
    .generateQRCode(String(fret.fretCode))
    .pipe(map(data => this.sanitizer.bypassSecurityTrustUrl(data)));
